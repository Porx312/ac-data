...
const SERVERS_PATH = _serversPath;
...
export const activeServers = loadPids();
/** Escribe server_cfg.ini / entry_list.ini según payload (misma lógica que la antigua API). */
export function applyServerConfiguration(serverName, payload) {
    const { displayName, password, track, configTrack, maxClients, entries, } = payload;
...
export function startServerCore(serverName) {
...
function execAsync(cmd) {
    return new Promise((resolve) => {
        exec(cmd, (err, stdout) => {
            const out = stdout ?? '';
            if (err)
                resolve({ stdout: out, err: err });
            else
                resolve({ stdout: out });
        });
    });
}
export async function stopServerCore(serverName) {
...
export async function restartServerCore(serverName) {
    const serverPath = path.join(SERVERS_PATH, serverName, 'acServer.exe');
    if (!fs.existsSync(serverPath)) {
